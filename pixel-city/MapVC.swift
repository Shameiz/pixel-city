...
class MapVC: UIViewController,UIGestureRecognizerDelegate {
    ...
    func addProgressLabel(){
        ...
        progressLabel?.frame = CGRect(x: (screenSize.width/2)-120, y: (pullUpViewHeightConstraint.constant/2)+25, width: 240, height: 40)
...
extension MapVC: MKMapViewDelegate{
    ...
    func retrieveUrls(forAnnotation annotation:DroppablePin, handler: @escaping CompletionHandler){
        imageUrlArray = []
        Alamofire.request(flickUrl(forApiKey: apiKey, withAnnotation: annotation, addNumberofPhotos: 40)).responseJSON { (response) in
            guard let json = response.result.value as? Dictionary<String,AnyObject> else{return}
            let phtosDict = json["photos"] as! Dictionary<String,AnyObject>
            let photosDictArray = phtosDict["photo"] as! [Dictionary<String,AnyObject>]
